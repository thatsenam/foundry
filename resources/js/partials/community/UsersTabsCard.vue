  ...
  <div class="col-span-full sm:col-span-6 xl:col-span-3 bg-white shadow-lg rounded-sm border border-slate-200">
    <div class="flex flex-col h-full">
      ...
      <div class="grow p-5">
        ...
        <header>
          <div class="flex justify-center mb-2">
            <router-link class="relative inline-flex items-start" :to="item.link">
          ...
          <div class="text-center">
            <router-link class="inline-flex text-slate-800 hover:text-slate-900" :to="item.link">
              <h2 class="text-xl leading-snug justify-center font-semibold">{{item.name}}</h2>
            </router-link>
          </div>
          <div class="flex justify-center items-center"><span class="text-sm font-medium text-slate-400 -mt-0.5 mr-1">-&gt;</span> <span>{{item.location}}</span></div>
        </header>
        <!-- Bio -->
      ...
      <div class="border-t border-slate-200">
        <router-link class="block text-center text-sm text-indigo-500 hover:text-indigo-600 font-medium px-3 py-4" to="/messages">
